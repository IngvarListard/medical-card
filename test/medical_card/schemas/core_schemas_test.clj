(ns medical-card.schemas.core-schemas-test
  (:require
   ...
   [medical-card.schemas.core-schemas :as schemas]
   [medical-card.schemas.utils :as subj]
   [medical-card.test.utils :refer [not-thrown?]]))

(deftest get-top-level-entries-test
  (testing "Test select only one field"
    (is (let [name-schema (subj/get-top-level-entries schemas/Research [:name])
              [[field-name & _]] name-schema]
          (and
           (= (count name-schema) 1)
           (= field-name :name)))))
  (testing "Test select multiple fields"
    (is (let [multi-schema (subj/get-top-level-entries schemas/Research [:name :description :type :start_date])]
          (= (count multi-schema) 4)))))


(deftest Research-test
  (testing "Test Reseach valid malli schema"
    (is (not-thrown? (partial m/schema schemas/Research)))))
